...
<b-container fluid>
    ...
    <b-container>
      <b-form>
        ...
        <b-row>
          <b-col id="date"> {{formatDate(noticeView.noticeDate)}}</b-col>
        </b-row>
        <b-row>
        </b-row>

          <b-row>
          <b-col > <pre id="content">{{noticeView.noticeContent}}</pre></b-col>
        </b-row>
...
export default {
    ...
    // 날짜변환 함수
      ...
      attachmentdown(attachment){
        location.href="http://localhost:8082/itjobgo/notice/filedownload?oriName="+attachment.originalfilename+"&reName="+attachment.renamedfilename;
      },
      //댓글삭제
      declick(commentno){
        ...
        return  this.$store.dispatch("FETCH_NT_COMMENT_LIST",this.$route.params.id);
        }else{
          return
        }
      },
