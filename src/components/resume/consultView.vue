      ...
      <div class="container-fluid">
        ...
        <div class="container">
          <b-col>
            <b-card class="viewcontainer">
              <b-form>
                <b-row>
                  <b-col class="rboardwriter"
                    >작성자 : {{ rboardDetail.rboardWriter }}<br
                  /></b-col>
                  <p class="rboarddate">
                    {{ formatDate(rboardDetail.rboardDate) }} 작성
                  </p>
                </b-row>

                <b-row>
                  <b-col class="consulttitle"
                    ><b>{{ rboardDetail.rboardTitle }}</b></b-col
                  >
                </b-row>
                <hr />
                <b-row>
                  <b-col class="consultcontent">{{
                    rboardDetail.rboardContent
                  }}</b-col>
                </b-row>
                <hr />
                <br />
                <b-row v-if="rboardAttachment.rboardNo != null">
                  ...
                  <b-col
                    ><b-button
                      class="qnaphotofile"
                      @click="rboardAttachmentDown(rboardAttachment)"
                      >{{ rboardAttachment.originalFilename }}</b-button
                    ></b-col
                  >
                </b-row>
              </b-form>
          ...
          <b-row>
            <b-col class="btndiv">
              ...
              <b-button
                v-if="userData.memberSq === rboardDetail.rboardId"
                class="btn_center"
                @click="updateConsult"
                >수정</b-button
              >
              <b-button
                v-if="
                  userData.memberSq === rboardDetail.rboardId ||
        ...
        <b-container>
                ...
                <b-row
                  ><b-col cols="2"
                    >{{ comment.memberName }} <br />{{
                      comment.rboardCommentDate | moment("YYYY.MM.DD HH:mm:ss")
                    }}
                  </b-col>
                  <!-- 쓴사람과 아닐떄는 일반 댓글로 보여주지않기 -->
                  <b-col v-if="comment.memberSq != userData.memberSq">{{
                    comment.rboardCommentContent
                  }}</b-col>
                      ...
                      <b-row>
                        <b-col>
                          <b-form-textarea
                            ...
                            :value="comment.rboardCommentContent"
                            @input="updateInput"
                            id="commentUptxt"
                          />
                        </b-col>
                            ...
                            <b-button
                              v-if="
                                userData.memberSq === comment.memberSq ||
                                  userData.memberEmail === 'admin@kh.com'
                              "
                              @click="declick(comment.rboardCommentno)"
                              id="deltet-btn"
                              >삭제</b-button
                            >
                            <b-button
                              @click="
                                upendclick(comment.rboardCommentno, $event)
                              "
                              id="updateEnd-btn"
                              >확인</b-button
                            >
          ...
          <b-form v-if="userData.memberSq != null">
            ...
            <b-row v-else>
              <p class="p1">
                이력서 컨설팅 전문가, 작성자, 관리자만 댓글 작성이 가능합니다.
              </p>
            </b-row>
          </b-form>
        </b-container>
  ...
  methods: {
    ...
    declick(commentno) {
      ...
      if (delfirm) {
        ...
        this.$store
          .dispatch("FETCH_RBOARD_COMMENTDEL", rboardCommentNo)
          .then(() => {
            this.$store.dispatch("FETCH_RBOARD_COMMENT", this.$route.params.id);
          });
      }
