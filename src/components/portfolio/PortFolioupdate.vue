      ...
      <b-container>
        <form @submit.prevent="test" enctype="multipart/form-data">
          ...
          <b-row align-h="center">
            <b-col cols="1">
              <b-button @click="test" type="submit" class="s-btn"
                >확인</b-button
              ></b-col
            >
            <b-col cols="1"
              ><b-button @click="back" class="r-btn">취소</b-button></b-col
            >
          </b-row>
        </form>
      </b-container>
...
export default {
  ...
  created() {
    ...
    this.$store.dispatch("FETCH_PBOARDUP", pboardNo).then(({ data }) => {
      console.log(data);
    });
    this.$store.dispatch("FETCH_PBOARDONE", pboardNo).then(({ data }) => {
