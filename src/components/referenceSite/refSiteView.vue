    ...
    <div class="container">
      <b-form>
        <b-row>
          ...
          <b-col id="title">
            <hr />
            {{ refListView.refTitle }}
            <p id="boardDate">
              No.{{ refListView.refNo }} {{ formatDate(refListView.refDate) }}
            </p>
            <hr />
          </b-col>
          <h2><hr /></h2>
        ...
        <b-btn
          ...
          id="linka"
          href=""
          v-on:click.stop.prevent="openWindow(refListView.refSiteAddr)"
        >
          사이트 바로가기</b-btn
        >
        <hr />
      ...
      <form @submit.prevent="updateStatus" enctype="multipart/form-data">
        <b-row class="bottom">
          <b-col>
            <b-row class="list">
              <b-button v-if="refListView.status === 'Y'" to="/refSite"
                >목록 이동</b-button
              >
              <b-button v-if="refListView.status === 'N'" to="/refSiteStatus"
                >목록 이동</b-button
              >
            </b-row>
            ...
            <b-row class="listupdel">
              <b-button
                class="adminbtn btn btn-success"
                v-if="
                  userData.memberEmail === 'admin@kh.com' &&
                    refListView.status === 'N'
                "
                type="submit"
                exact
                >사이트 승인하기</b-button
              ...
              <b-button
                v-if="userData.memberSq === refListView.memberNum"
                @click="update"
                id="updatebutton"
                >수정</b-button
              >
              <b-button
                v-if="
                  userData.memberSq === refListView.memberNum ||
                    userData.memberEmail === 'admin@kh.com'
                "
                @click="pdelete"
